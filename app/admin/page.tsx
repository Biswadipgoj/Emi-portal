  ...
  return (
    <div className="min-h-screen gradient-bg">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Tab Navigation */}
        <div className="flex items-center gap-1 mb-8 bg-obsidian-900/60 rounded-2xl p-1.5 border border-white/[0.05] w-fit">
          {([
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${
                tab === t.key ? 'bg-gold-500 text-obsidian-950 shadow-lg shadow-gold-500/20' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              {t.label}
            </button>
          ...
          <div className="space-y-6 animate-fade-in">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-slate-500 text-sm mt-1">Search all customers — RUNNING and COMPLETE</p>
              </div>
              <button onClick={() => { setEditingCustomer(null); setShowCustomerForm(true); }} className="btn-gold">
                + New Customer
              </button>
            </div>
            ...
            {searchResults === null && (
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <p className="text-slate-500 text-lg">Enter name, IMEI, or Aadhaar to search</p>
                <p className="text-slate-700 text-sm mt-1">Results appear as you type — no button needed</p>
              </div>
            )}

            {searchResults !== null && searchResults.length === 0 && (
              <div className="text-center py-16">
                <p className="text-slate-500">No customers found. Try a different search term.</p>
              </div>
            )}
            ...
            {searchResults !== null && searchResults.length > 1 && !selectedCustomer && (
              <div className="glass-card overflow-hidden animate-fade-in">
                <div className="px-5 py-3 border-b border-white/[0.05]">
                  <span className="text-xs text-slate-400 uppercase tracking-widest">{searchResults.length} customers found — click a row to view</span>
                </div>
                  ...
                  <tbody>
                    {searchResults.map((c) => (
                      <tr key={c.id} onClick={() => selectCustomerFn(c)} className="cursor-pointer">
                        ...
                        <td><span className="text-slate-400">{(c.retailer as Retailer)?.name || '—'}</span></td>
                        <td>
                          {c.status === 'RUNNING'
          ...
          <div className="space-y-6 animate-fade-in">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-slate-500 text-sm mt-1">{retailers.length} retailers registered</p>
              </div>
              <button
                ...
                <tbody>
                  {retailers.map((r) => (
                    <tr key={r.id}>
                      ...
                      <td><span className="font-num text-slate-400">@{r.username}</span></td>
                      <td>{r.is_active ? <span className="badge-running">Active</span> : <span className="badge-rejected">Inactive</span>}</td>
                      <td className="text-xs text-slate-500">{format(new Date(r.created_at), 'd MMM yyyy')}</td>
                      <td>
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="px-3 py-1 text-xs border border-white/10 hover:border-gold-500/30 hover:text-gold-400 rounded-lg transition-colors text-slate-400"
                          >
                            Edit
                          </button>
                          <button
                  ...
                  {retailers.length === 0 && (
                    <tr><td colSpan={5} className="text-center text-slate-600 py-10">No retailers yet. Add one to get started.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="glass-card p-6">
              ...
              <div className="flex items-end gap-4">
                ...
                <button onClick={updateFineSettings} className="btn-gold">Save</button>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {[
                  ...
                  <button
                    ...
                    className="px-4 py-3 rounded-xl border border-white/[0.08] hover:border-gold-500/30 text-slate-400 hover:text-gold-400 text-sm font-medium transition-all text-left flex items-center gap-2"
                  >
            ...
            <div className="glass-card p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                {activeFilter && (
                  <button onClick={clearFilter} className="text-xs text-slate-500 hover:text-slate-300 underline underline-offset-4 transition-colors">
                    Clear filter
                  </button>
                )}
              </div>
              ...
              {/* Upcoming due */}
              <p className="text-xs text-slate-500 mb-2 uppercase tracking-widest">Upcoming due date</p>
              ...
              {/* Overdue by months */}
              <p className="text-xs text-slate-500 mb-2 uppercase tracking-widest">Overdue by months</p>
              <div className="flex flex-wrap gap-2 mb-5">
                {[2, 3, 4, 5].map((m) => (
                  <button
                    key={m}
              ...
              {filteredEmis !== null && !filterLoading && (
                <div className="mt-6">
                  ...
                  {filteredEmis.length === 0 ? (
                    <p className="text-slate-600 text-sm py-4">No EMIs match this filter.</p>
                  ) : (
                    <div className="glass-card overflow-hidden">
                      <table className="data-table">
                        <thead>
                        ...
                        <tbody>
                          {filteredEmis.map((row) => (
                            <tr key={row.id}>
                              ...
                              <td><span className="font-num text-slate-400">{row.mobile}</span></td>
                              <td className="text-slate-400">{row.retailer_name}</td>
                              <td><span className="font-num">#{row.emi_no}</span></td>
                              <td>
                                <span className={`font-num text-xs ${new Date(row.due_date) < new Date() ? 'text-crimson-400 font-semibold' : 'text-slate-400'}`}>
      ...
      {showDeleteConfirm && (
        <div className="modal-backdrop">
          <div className="glass-card w-full max-w-md p-6 animate-slide-up">
            <h3 className="font-display text-xl font-bold text-crimson-400 mb-2">⚠ Delete Customer</h3>
            <p className="text-sm text-slate-400 mb-5">
          ...
          <div className="glass-card w-full max-w-md p-6 animate-slide-up">
            ...
            <form onSubmit={handleRetailerSubmit} className="space-y-4">
              ...
              {!editingRetailer && (
                <div>
                  ...
                  <input
                    ...
                    className="form-input"
                  />
                  <p className="text-xs text-slate-600 mt-1">Login email will be: {retailerForm.username || 'username'}@retailer.local</p>
                </div>
              )}
              <div>
                ...
                />
                {!editingRetailer && (
                  <p className="text-xs text-slate-600 mt-1">Used to log in to the retailer dashboard.</p>
                )}
              </div>
              <div>
                <label className="form-label">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" className="btn-gold flex-1">{editingRetailer ? 'Update Retailer' : 'Create Retailer'}</button>
              </div>
            </form>
          </div>
        </div>
