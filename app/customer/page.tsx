        ...
        <div className="relative w-full max-w-md animate-slide-up">
          ...
          <div className="glass-card p-8 shadow-2xl shadow-black/40">
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              ...
              <button type="submit" disabled={loading || aadhaar.length !== 12 || mobile.length !== 10} className="btn-gold w-full py-3.5 text-base mt-2">
...
function Field({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div>
      ...
      <p className={`text-sm text-slate-200 ${mono ? 'font-num' : ''}`}>{value || '—'}</p>
    </div>
  );
}
